...
void ResetDialog::on_varify_btn_clicked()
{
    ...
    HttpMgr::GetInstance()->PostHttpReq(QUrl(gate_url_prefix+"/get_verifycode"),
                                        json_obj, ReqId::GET_VARIFT_CODE_ID,Modules::RESETMOD);
}

void ResetDialog::slot_reset_mod_finish(ReqId id, QString res, ResCodes err)
{
    if(err != SUCEESS){
...
bool ResetDialog::checkPassValid()
{
    ...
    if(pass.length() < 6 || pass.length()>15){
        //提示长度不准确
        AddTipErr(Tip_Pwd, tr("密码长度应为6~15"));
        return false;
    }
...
bool ResetDialog::checkEmailValid()
{
    ...
    if(!match){
        //提示邮箱不正确
        AddTipErr(Tip_Email, tr("邮箱地址不正确"));
        return false;
    }
...
bool ResetDialog::checkVarifyValid()
{
    ...
    if(pass.isEmpty()){
        AddTipErr(Tip_VerifyCode, tr("验证码不能为空"));
        return false;
    }

    DelTipErr(Tip_VerifyCode);
    return true;
}

void ResetDialog::AddTipErr(ErrTips te, QString tips)
{
    _tip_errs[te] = tips;
    showTip(tips, false);
}

void ResetDialog::DelTipErr(ErrTips te)
{
    _tip_errs.remove(te);
    if(_tip_errs.empty()){
      ui->error_tip->clear();
...
void ResetDialog::initHandlers()
{
    ...
    _handlers.insert(ReqId::ID_RESET_PWD, [this](QJsonObject jsonObj){
        ...
        qDebug()<< "email is " << email ;
        //qDebug()<< "user id is " <<  jsonObj["uuid"].toString();
    });
}
