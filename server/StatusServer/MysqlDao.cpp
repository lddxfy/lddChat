...
int MysqlDao::CheckLogin(const std::string& email,const std::string& pwd)
{
    auto con = pool_->GetConnection();
    if (con == nullptr)  return false;
    try
    {
        // 准备查询语句
        std::unique_ptr < sql::PreparedStatement > stmt(con->prepareStatement("Select pwd from users where email = ?"));
        //指定参数
        stmt->setString(1, email);
        //获取结果集
        std::unique_ptr<sql::ResultSet> res(stmt->executeQuery());
